...
import {
  ...
  pixelArtsItems, pixelArtsTerrains, terrainDefs,
} from '../../game/data/gameData';
import { playSound } from '../../game/data/gameAudio';
...
const GameBoardDev = forwardRef(function GameBoardDev({ onStateChange }, ref) {
    ...
    /* ── Broadcast state to DevToolsPanel ── */
    const broadcastState = useCallback(() => {
        ...
        onStateChange({
            ...
            isPaused: s.isPaused,
            activeTerrain: s.activeTerrain ? { id: s.activeTerrain.id, turns: s.activeTerrain.turns, owner: s.activeTerrain.owner } : null,
            p1: {
                activeCard: s.p1.activeCard ? { name: s.p1.activeCard.name, atk: s.p1.activeCard.atk, def: s.p1.activeCard.def, pa: s.p1.activeCard.pa } : null,
                cardsLeft: s.p1.cardsLeft,
    ...
    const updateUI = useCallback(() => {
        let state = stateRef.current;
        const el = (id) => $(`${id}`);
        const p1DeckCount = el('p1-deck-count'); const p2DeckCount = el('p2-deck-count');
        const p1WeaponCount = el('p1-weapon-count'); const p2WeaponCount = el('p2-weapon-count');
        ...
        const p1Slot = el('p1-slot'); const p2Slot = el('p2-slot');
        ...
        for(let i=0; i<3; i++) {
            const p1w = el(`p1-wslot-${i}`); const p2w = el(`p2-wslot-${i}`);
        ...
        const tInd = el('turn-indicator'); const s1 = el('p1-section'); const s2 = el('p2-section');
        if (state.turn === 1) {
            ...
            const p1Deck = el('p1-deck'); const p1WeaponDeck = el('p1-weapon-deck');
            ...
            const p2Deck = el('p2-deck'); const p2WeaponDeck = el('p2-weapon-deck');
    ...
    const togglePause = useCallback(() => {
        ...
        const pauseMenu = $('pause-menu');
        if (!pauseMenu) return;
        if (state.isPaused) { pauseMenu.classList.remove('hidden'); pauseMenu.classList.add('flex'); }
        else { pauseMenu.classList.add('hidden'); pauseMenu.classList.remove('flex'); }
        updateUI();
    }, [$, updateUI]);

    const showTutorial = useCallback(() => {
        const pauseMenu = $('pause-menu'); const tutorialOverlay = $('tutorial-overlay');
        if (pauseMenu) { pauseMenu.classList.add('hidden'); pauseMenu.classList.remove('flex'); }
    ...
    const rebindWeaponHandlers = useCallback(() => {
        ...
        for (let playerNum = 1; playerNum <= 2; playerNum++) {
            for (let i = 0; i < 3; i++) {
                ...
                if (el) { el.onclick = () => equipWeaponRef.current(playerNum, i); el.oncontextmenu = (e) => discardWeaponRef.current(e, playerNum, i); }
            }
        }
    }, [$]);
    ...
    const rebindKnightHandlers = useCallback(() => {
        ...
        for (let playerNum = 1; playerNum <= 2; playerNum++) {
            ...
            if (pState.activeCard) {
                ...
                if (el) { el.onclick = () => handleKnightClick(playerNum); el.oncontextmenu = (e) => handleRightClick(e, playerNum); }
            }
        }
    ...
    const initGame = useCallback(async () => {
        ...
        if (deckState?.knights?.length && deckState?.cards?.length) {
            ...
            state.p1.deck = knights; state.p1.weaponDeck = equipment; state.p1.cardsLeft = knights.length; state.p1.weaponsLeft = equipment.length;
        } else {
            state.p1.deck = createDeck(generateCard, 5); state.p1.weaponDeck = generateEquipmentDeck(); state.p1.cardsLeft = 5; state.p1.weaponsLeft = 45;
        }
        ...
        try { updateUI(); await wait(500); drawCard(1, true); await wait(600); drawCard(2, true); await wait(400); }
        catch (err) { console.error('[GameBoardDev] initGame error:', err); }
    ...
    useEffect(() => {
        const handler = (e) => {
            ...
            if (e.key === 'Escape' || e.key === 'Esc') {
                if (tutorialOverlay && !tutorialOverlay.classList.contains('hidden')) hideTutorial();
                else if (!state.gameOver && !state.isInitializing) togglePause();
            }
        };
    ...
    /* ══════ DEV IMPERATIVE HANDLE ══════ */
    useImperativeHandle(ref, () => ({
        getState: () => stateRef.current,
        restart: () => { initRef.current = false; setTimeout(() => { initRef.current = true; initGame(); }, 50); },
        forceDrawP1: () => drawCard(1, true),
        forceDrawP2: () => drawCard(2, true),
        maxPA: (playerNum) => {
            const p = playerNum === 1 ? stateRef.current.p1 : stateRef.current.p2;
            if (p.activeCard) { p.activeCard.pa = 5; updateUI(); }
        },
        instaKill: (playerNum) => {
            const p = playerNum === 1 ? stateRef.current.p1 : stateRef.current.p2;
            if (p.activeCard) { p.activeCard.def = 0; p.activeCard = null; p.buffs = []; updateUI(); checkWinCondition(); }
        },
        forceTerrain: (terrainId) => {
            const def = terrainDefs.find(t => t.id === terrainId);
            if (!def) return;
            const card = {
                id: 't_dev_' + Math.random().toString(36).substr(2, 9),
                type: 'terreno', itemId: def.id, name: def.name, cu: 0,
                desc: def.desc, art: getPixelSVG(pixelArtsTerrains[0]),
            };
            activateTerrain(card, 1);
        },
        triggerAI: () => { if (!stateRef.current.gameOver) playAITurnRef.current(); },
        setTurn: (t) => { stateRef.current.turn = t; stateRef.current.hasAttacked = false; updateUI(); },
    }), [drawCard, updateUI, checkWinCondition, activateTerrain, initGame]);

    // --- RENDER ---
    return (
        <div ref={containerRef} className="game-page h-full flex flex-col" style={{ fontFamily: "'Press Start 2P', monospace" }}>
            <header className="py-2 md:py-4 border-b-2 border-red-900 bg-black/60 relative z-10 flex flex-row items-center justify-between px-4 md:px-8">
                ...
                <div className="flex flex-col items-center flex-grow">
                    <h1 className="text-xl md:text-4xl text-red-700 frazetta-title mb-1 md:mb-2 tracking-widest leading-none">DEV — Cavalieri Oscuri</h1>
            ...
            <div id="tutorial-overlay" className="fixed inset-0 hidden z-[120] bg-black/95 flex-col justify-start items-center p-4 sm:p-8 overflow-y-auto">
                ...
                <div className="max-w-3xl w-full bg-[#111] border-4 border-red-900 p-4 md:p-8 text-[0.55rem] sm:text-[0.7rem] text-gray-300 leading-relaxed md:leading-loose space-y-4 md:space-y-6 overflow-y-auto custom-scrollbar flex-grow" style={{boxShadow: '8px 8px 0px #000'}}>
                    <div><h3 className="text-red-500 text-sm md:text-lg mb-1 md:mb-2">{'\uD83D\uDEE1\uFE0F'} Scopo del Gioco</h3><p>Sconfiggi tutti i Cavalieri del tuo avversario!</p></div>
                    <div><h3 className="text-red-500 text-sm md:text-lg mb-1 md:mb-2">{'\u2694\uFE0F'} Fasi del Turno</h3>
                        <ul className="list-none space-y-2 md:space-y-3">
                            <li><span className="text-yellow-500">{'>'}</span> <strong>SCHIERA:</strong> Clicca sul MAZZO.</li>
                            <li><span className="text-yellow-500">{'>'}</span> <strong>RIFORNISCI:</strong> Clicca sulla RASTRELLIERA (1x turno).</li>
                            <li><span className="text-yellow-500">{'>'}</span> <strong>EQUIPAGGIA:</strong> Tocca/Clicca un arma/scudo/oggetto.</li>
                            <li><span className="text-yellow-500">{'>'}</span> <strong>COMBATTI:</strong> ATK vs DEF.</li>
                        </ul>
                    </div>
                </div>
